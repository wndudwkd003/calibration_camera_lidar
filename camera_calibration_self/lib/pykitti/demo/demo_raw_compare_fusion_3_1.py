...
def update_fig(idx):
    ...
    rgb_shape = rgb.shape
    lidar_mat = np.full((rgb_shape[0], rgb_shape[1], 3), 255, dtype=np.uint8)  # 흰색 바탕의 새 이미지 생성
    ...
    velo_copy[:, 3] = 1  # 동차 좌표 추가
    velo_image = dataset.calib.T_cam2_velo.dot(velo_copy.T)
    ...
    for i in range(velo_image.shape[1]):
        if velo[i][3] >= 0.3:
            ...
            if 0 <= x < rgb.shape[1] and 0 <= y < rgb.shape[0]:
                ...
                cv2.circle(lidar_mat, (x, y), 1, (0, 0, 255), -1)  # 빨간색으로 표시


    # 이미지 표시
    cv2.imshow('RGB Image', rgb)
    cv2.imshow('LiDAR on RGB Image', rgb_fusion)
    cv2.imshow('Stereo Disparity Map', disp_rgb_color)
    cv2.imshow('Filtered Stereo Disparity Map', filtered_disp_color)
    # lidar_mat = cv2.flip(lidar_mat, 0)
    cv2.imshow('LiDAR on White Background', lidar_mat)
