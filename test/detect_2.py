...
def draw_axes_for_tag(image, tag):
    # 회전 행렬을 회전 벡터로 변환
    ...
    axis_points = np.float32([
        ...
        [axis_length, 0, 0],  # x축
        [0, axis_length, 0],  # y축
        [0, 0, -axis_length]  # z축 (카메라 방향을 고려하여 음수로 설정)
    ]).reshape(-1, 3)
    ...
    # 축 그리기
    cv2.line(image, tuple(map(int, imgpts[0].ravel())), tuple(map(int, imgpts[1].ravel())), (255, 0, 0),
             thickness)  # x축: 빨간색
    cv2.line(image, tuple(map(int, imgpts[0].ravel())), tuple(map(int, imgpts[2].ravel())), (0, 255, 0),
             thickness)  # y축: 초록색
    cv2.line(image, tuple(map(int, imgpts[0].ravel())), tuple(map(int, imgpts[3].ravel())), (0, 0, 255),
             thickness)  # z축: 파란색
